...
const Dashboard = () => {
  ...
  if (loading)
    return (
      <p style={{ textAlign: "center", color: "white" }}>Loading books...</p>
    );
  if (error)
            ...
            <Link key={book.id} to={`/book/${book.id}`} className="book-card">
              <img
                src={
                  ...
                  "https://via.placeholder.com/300x400/065f46/ffffff?text=📚+Book+Cover"
                }
                alt={book.title}
                onError={(e) => {
                  e.target.src =
              ...
              <h3 style={{ color: "white", margin: "1rem 0 0.5rem 0" }}>
                {book.title}
              </h3>
              <p
                style={{
